...
describe('Leagues Invitations', () => {
  ...
  it(`GET /me/league-invitations 200 A user can see their league invitations`, async () => {
    ...
    expect(list.json().results[0].to_user.id).toBe(user2.id)
    expect(list.json().results[0].league.image).toBeDefined()
    expect(list.json().results[0].league.team).toBeDefined()
    expect(list.json().results[0].league.sport).toBeDefined()
    expect(list.json().results[0].league.organisation).toBeDefined()
  })
})
